...
const ManagementDashboard = () => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      return response.buttonPermissions;
    },
  });
  ...
  const updateButtonPermissionMutation = useMutation({
    mutationFn: async ({ permissionId, updatedData }) => {
      ...
      //   }
      // );
      const response = await fetchUserControlById(permissionId);
      return response;
    },
    onSuccess: () => {
  ...
  const updatePagePermissionMutation = useMutation({
    mutationFn: async ({ permissionId, updatedData }) => {
      ...
      //   }
      // );
      const response = await fetchUserControlNew(permissionId);
  ...
  const {
    ...
    queryFn: async () => {
      ...
      return response.pagePermissions;
    },
  });
  ...
  const getUserTypeInfo = (userTypeId) => {
    const types = {
      1: { label: "Student", classes: "bg-blue-100 text-blue-800" },
