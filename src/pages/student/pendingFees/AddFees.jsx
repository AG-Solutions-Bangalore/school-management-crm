...
const inputClass =
  ...
  export const AddFees = ({ open, handleOpen }) => {
    ...
    const fetchData = async () => {
      ...
      const fetchPendingFees = async () => {
        try {
          ...
          if (!formData.studentFees_year || !formData.studentFees_class || !formData.studentFees_admission_no) {
          ...
          if (response.data && response.data.student && response.data.student.length > 0) {
    ...
    return (
          ...
          <DialogTitle className="flex flex-col md:flex-row items-center justify-between">
            ...
            <div>
              ...
              >
               ...
               <Select
      name="studentFees_admission_no"
      value={students.find(student => student.student_admission_no === formData.studentFees_admission_no)
        ? {
            value: formData.studentFees_admission_no,
            label: `${students.find(student => student.student_admission_no === formData.studentFees_admission_no).student_name} (${formData.studentFees_admission_no})`
          }
        : null
      }
      onChange={(selectedOption) => {
        setFormData({
          ...formData,
          studentFees_admission_no: selectedOption ? selectedOption.value : '',
        });
      }}
  ...
  <div className="flex flex-row items-center gap-3 text-xs">
    {loading ? (
      ...
      <>
        <span>Total: <b>₹{pendingFeesData.total_amount}</b></span>
        <span className="text-green-600">Paid: <b>₹{pendingFeesData.paid_amount}</b></span>
        <span className="text-red-600">Balance: <b>₹{pendingFeesData.total_amount - pendingFeesData.paid_amount}</b></span>
        {pendingFeesData.van_required === "Yes" && (
          <span>Van: <b>₹{pendingFeesData.van_amount}</b></span>
        )}
      </>
    ) : (
      formData.studentFees_admission_no && (
