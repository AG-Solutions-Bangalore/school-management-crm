...
const FeeSummary = () => {
  ...
  const classOptions = ["NURSERY", "LKG", "UKG", "I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX", "X"];

  // Close dropdowns when clicking outside
  const closeDropdowns = useCallback((e) => {
    if (!e.target.closest('#classFilterDropdown') && !e.target.closest('.class-filter-button')) {
      setShowClassFilter(false);
    }
    if (!e.target.closest('#feeFilterDropdown') && !e.target.closest('.fee-filter-button')) {
      setShowFeeFilter(false);
    }
    if (!e.target.closest('#columnFilterDropdown') && !e.target.closest('.column-filter-button')) {
  ...
  const sortedClasses = classOptions.filter(className => classGroupedData[className])
  ...
  // Function  download CSV --sajid[444]
  const downloadCSV = async () => {
      ...
      sortedClasses.forEach(className => {
          if (selectedClasses.length === 0 || selectedClasses.includes(className)) {
              const filteredStudents = applyFilters(classGroupedData[className], className);
            ...
            worksheet.mergeCells(headerRow.number, 1, headerRow.number, columnCount);
            ...
            worksheet.mergeCells(blankRow1.number, 1, blankRow1.number, columnCount);
            worksheet.mergeCells(blankRow2.number, 1, blankRow2.number, columnCount);
          }
      });
      ...
      <div className="p-4 bg-white rounded-lg shadow-md">
        <div className="mb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
          <h2 className="text-xl font-bold text-gray-800">
            Fee Summary <span className="text-gray-500 text-base font-normal">{selectedYear}</span>
          </h2>
        ...
        <div className="mb-3 flex flex-wrap gap-2">
          ...
          <div className="relative">
            ...
            >
              Class Filter {selectedClasses.length > 0 && renderFilterBadge(selectedClasses.length)}
              <svg className="w-3 h-3 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path>
              </svg>
            </button>
            {showClassFilter && (
              <div id="classFilterDropdown" className="absolute left-0 z-10 mt-1 w-44 bg-white rounded shadow-lg border border-gray-100">
                <div className="p-2">
                  ...
                  <div className="max-h-48 overflow-y-auto">
                    {classOptions.map((className) => (
                      <label key={className} className="flex items-center px-2 py-1 text-sm text-gray-700 hover:bg-gray-50 cursor-pointer">
                        <input
                          type="checkbox"
          ...
          <div className="relative">
            ...
            {showFeeFilter && (
              <div id="feeFilterDropdown" className="absolute left-0 z-10 mt-1 w-40 bg-white rounded shadow-lg border border-gray-100">
                <div className="py-1">
                  <button
                    onClick={() => {setFeeFilter('all'); setShowFeeFilter(false);}}
                    className={`block w-full text-left px-3 py-1 text-sm ${feeFilter === 'all' ? 'bg-indigo-50 text-indigo-700' : 'text-gray-700 hover:bg-gray-50'}`}
                  >
                    All Students
                  </button>
                  <button
                    onClick={() => {setFeeFilter('zeroPay'); setShowFeeFilter(false);}}
                    className={`block w-full text-left px-3 py-1 text-sm ${feeFilter === 'zeroPay' ? 'bg-indigo-50 text-indigo-700' : 'text-gray-700 hover:bg-gray-50'}`}
                  >
                    No Payment
                  </button>
                  <button
                    onClick={() => {setFeeFilter('onePay'); setShowFeeFilter(false);}}
                    className={`block w-full text-left px-3 py-1 text-sm ${feeFilter === 'onePay' ? 'bg-indigo-50 text-indigo-700' : 'text-gray-700 hover:bg-gray-50'}`}
                  >
                    1 Time Pay
                  </button>
                  <button
                    onClick={() => {setFeeFilter('paid'); setShowFeeFilter(false);}}
                    className={`block w-full text-left px-3 py-1 text-sm ${feeFilter === 'paid' ? 'bg-indigo-50 text-indigo-700' : 'text-gray-700 hover:bg-gray-50'}`}
                  >
                    Paid
                  </button>
                  <button
                    onClick={() => {setFeeFilter('pending'); setShowFeeFilter(false);}}
                    className={`block w-full text-left px-3 py-1 text-sm ${feeFilter === 'pending' ? 'bg-indigo-50 text-indigo-700' : 'text-gray-700 hover:bg-gray-50'}`}
                  >
                    Pending
                  </button>
                </div>
          ...
          <div className="relative">
            ...
            >
              Column Filter {getColumnVisibilityCount() > 0 && renderFilterBadge(getColumnVisibilityCount())}
              <svg className="w-3 h-3 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path>
              </svg>
            </button>
            {showColumnFilter && (
              <div id="columnFilterDropdown" className="absolute left-0 z-10 mt-1 w-40 bg-white rounded shadow-lg border border-gray-100">
                <div className="py-1">
                  {Object.keys(visibleColumns).map((column) => (
                    <label key={column} className="flex items-center px-3 py-1 text-sm text-gray-700 hover:bg-gray-50 cursor-pointer">
                      <input
                        type="checkbox"
                      ...
                      {column === 'slNo' ? 'Sl No' : 
                       ...
                       column === 'paymentsCount' ? 'No. of Payments' : 
          ...
          <div className="mb-3 flex flex-wrap gap-1.5">
            {feeFilter !== 'all' && (
              <span className="inline-flex items-center px-2 py-1 bg-indigo-50 text-indigo-700 text-sm rounded">
                {feeFilter === 'zeroPay' ? 'No Payment' : 
                 feeFilter === 'onePay' ? '1 Time Pay' : 
                 feeFilter === 'paid' ? 'Paid' : 'Pending'}
                <button 
            ...
            {selectedClasses.length > 0 && (
              <span className="inline-flex items-center px-2 py-1 bg-indigo-50 text-indigo-700 text-sm rounded">
                {selectedClasses.length === 1 ? `Class: ${selectedClasses[0]}` : 
        ...
        <div ref={containerRef} className="print:text-sm">
          {sortedClasses.map((className) => {
            ...
            const totalFees = filteredStudents.reduce((sum, student) => sum + student.totalAmount, 0);
            const totalPaid = filteredStudents.reduce((sum, student) => sum + student.paidAmount, 0);
            const totalBalance = filteredStudents.reduce((sum, student) => sum + student.balance, 0);
            
            return (
              <div key={className} className="mb-4 overflow-hidden bg-white border border-gray-200 rounded shadow-sm">
                <div className="flex justify-between items-center px-3 py-2 bg-gray-50 border-b border-gray-200">
                  ...
                  <div className="text-sm flex gap-3 text-gray-800">
                    ...
                    <span className="text-green-800">Paid: ₹{totalPaid.toLocaleString()}</span>
                    <span className="text-red-800">Balance: ₹{totalBalance.toLocaleString()}</span>
                  </div>
                </div>
                        ...
                        >
                          ...
                          {visibleColumns.paidAmount && (
                            <td className="p-2 text-sm text-right">
                              <span className={`${student.paidAmount === 0 ? 'text-red-800' : 'text-green-800'}`}>
                          ...
                          {visibleColumns.balance && (
                            <td className="p-2 text-sm text-right">
                              <span className={`${student.balance > 0 ? 'text-red-800' : 'text-green-800'}`}>
                          ...
                          {visibleColumns.paymentsCount && (
                            <td className="p-2 text-sm text-center">
                              <span className={`inline-flex items-center justify-center px-1.5 py-0.5 text-sm font-medium rounded
        ...
        {Object.keys(classGroupedData).length > 0 && 
         Object.keys(classGroupedData).every(className => applyFilters(classGroupedData[className], className).length === 0) && (
